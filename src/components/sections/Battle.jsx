...
export default function Battle() {
  ...
  useEffect(() => {
    ...
    botTimer.current = window.setTimeout(() => {
      setGame(current => {
        ...
        const incorrectAnswers = currentQuestion.options.map((_, index) => index).filter(index => index !== currentQuestion.correctIndex)
        const botSelected = Math.random() < .75
          ? currentQuestion.correctIndex
          : incorrectAnswers[Math.floor(Math.random() * incorrectAnswers.length)]
        return { ...current, botSelected, botScore: current.botScore + (botSelected === currentQuestion.correctIndex ? 100 : 0) }
      })
    }, 2200)
  ...
  const selectAnswer = selected => {
    ...
    setGame(current => ({ ...current, selected, userScore: current.userScore + (selected === question.correctIndex ? 100 : 0) }))
  }
